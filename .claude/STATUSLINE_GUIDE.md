...
臨時在 `.claude/settings.json` 的 statusline command 前面加 `tee /tmp/statusline-in.json |`：

```json
"command": "tee /tmp/statusline-in.json | bash .claude/statusline.sh"
```

互動一次後查看：

```bash
cat /tmp/statusline-in.json
```
