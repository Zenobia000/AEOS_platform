...
> **底層 runtime**：nanobot（Python，AEOS 凍結+多租戶+Tool Gateway 包覆，ADR-0001）。
...
| ADR | `docs/architecture/adr/ADR-0001`(nanobot runtime)・`0002`(vertical pack)・`0003`(結構化 contact) |
...
W2  ③ 結構化 contact（活檔案 7 欄位 + 時間軸，ADR-0003）+ KnowledgeRouter 兩路
...
## 4. 不可違反的鐵律
1. 學習/生產分離（Frozen，ADR-0001）2. 草稿模式（AI 不自動發）3. Kill switch 30s 4. Audit 全覆蓋 5. 跨租戶 0 串（RLS，紅隊必過）6. 外送 0 踩線（合規 gate）
...
ADR-0001/0002/0003（Proposed）、NFR、C4、System Spec、User Flow、OpenAPI、ERD、Test Plan、Runbook、Release Readiness 皆 draft/proposed，等 multi-role review + 業主簽核轉 frozen。
