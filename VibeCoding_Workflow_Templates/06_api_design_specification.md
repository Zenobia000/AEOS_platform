...
> **版本:** v1.0 | **更新:** YYYY-MM-DD | **狀態:** 草稿/已發布
> **負責人:** TL | **審核:** ARCH + SEC | **追蹤:** US-NNNN (使用者故事), ADR-NNNN (技術決策)
> **OpenAPI 定義:** [連結]
...
業務級錯誤（如「餘額不足」「配額超限」）使用 snake_case，命名規則 `<resource>_<reason>`：

| 錯誤碼 | HTTP | 描述 |
| :--- | :--- | :--- |
| `wallet_insufficient_balance` | 402 / 422 | 錢包餘額不足 |
| `quota_exceeded` | 429 | 用量配額超限 |
| `subscription_required` | 402 | 需訂閱方案 |
| `resource_locked` | 423 | 資源已被鎖定（編輯衝突） |

回傳格式同 §3，`code` 欄填業務碼，`type` 欄統一為 `business_rule_error`。

---

## 4. 安全性

> 📎 **與 13 §C-API 的邊界**: 本檔聚焦 **API 契約層面** 的安全（認證機制、速率限制 header、錯誤碼）。**實作層面** 的安全要求（密碼 hash、Session 設定、輸入驗證實作、CVE 修補）見 `13_security_and_readiness_checklists.md §C`。
...
- **OWASP API Top 10**: 已考量並緩解（驗證清單見 13 §C-API）
